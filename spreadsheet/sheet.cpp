...
void Sheet::UpdateDependencies(Position pos, const std::vector<Position>& old_refs, const std::vector<Position>& new_refs) {
    ...
    // удал€ем зависимости, которые есть в старых ссылках, но отсутствуют в новых
    ...
    // добавл€ем зависимости, которые есть в новых ссылках, но отсутствуют в старых
...
void Sheet::AddDependency(Position from, Position to) {
    ...
    if (cell) {
        cell->AddDependentCell(to);  // добавление зависимой €чейки
    }
}
...
void Sheet::RemoveDependency(Position from, Position to) {
    auto* cell = dynamic_cast<Cell*>(GetCell(from));
    if (cell) {
        cell->RemoveDependentCell(to);  // удаление зависимой €чейки
    }
}   
...
void Sheet::SetCell(Position pos, std::string text) {
    ...
    if (text[0] == '=') {
        try {
            ...
            // проверка каждой ссылку на €чейку в формуле
...
void Sheet::ClearCell(Position pos) {
    ...
    if (it != sheet_.end()) {
        ...
        // провер€ем, есть ли ссылки на эту €чейку из других €чеек
        ...
        // если на €чейку нет ссылок и она пуста€, удал€ем еЄ
...
void Sheet::PrintValues(std::ostream& output) const {
    ...
    for (int row = 0; row < size.rows; ++row) {
        for (int col = 0; col < size.cols; ++col) {
            ...
            const CellInterface* cell = GetCell(pos); // €чейка по позиции
            if (cell) {
                std::visit([&output](const auto& value) { output << value; }, cell->GetValue());
            }

            if (col < size.cols - 1) {
                output << '\t';  // разделитель табул€ции между столбцами
            }
        }
...
void Sheet::PrintTexts(std::ostream& output) const {
    ...
    for (int row = 0; row < size.rows; ++row) {
        for (int col = 0; col < size.cols; ++col) {
            Position pos{ row, col };
            const CellInterface* cell = GetCell(pos); // получаем €чейку по позиции
            if (cell) {
                output << cell->GetText();
            }
